...
<div id="postlist" class="pl">
		...
		<table cellspacing="0" cellpadding="0">
			...
			<!--{if $_G['forum_thread']['replycredit'] > 0 }-->
				<tr>
					...
					<td class="pls vm ptm">
					<!--{else}-->
					<td class="pls ptm pbm xi1 bm" colspan="2">
					<!--{/if}-->
	...
	<!--{loop $postlist $post}-->
		<!--{if $rushreply && $_GET['checkrush'] && $post['rewardfloor'] != 1}-->
			<!--{eval continue;}-->
		<!--{/if}-->
		<div id="post_$post[pid]" class="bm" {if $_G['blockedpids'] && $post['inblacklist']}style="display:none;"{/if}>
